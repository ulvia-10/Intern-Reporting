...
class Auth extends CI_Controller
{
	...
	private function _sendEmail($token, $type)
	{
		...
		if ($type == 'verify') {
			...
			$this->email->message('Open This Message To Verify Email : 		
			<a href="' . base_url() . 'auth/verify?email=' . $this->input->post('email') . '&token=' . urlencode($token) . '"><button type="button" class="btn btn-warning">Open</button></a>');
		} else if ($type == 'forget') {
	...
	public function pageemail()
	{
		...
		$this->load->view('templates/email_header', $data);
	...
	public function forgetpassword()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			user', ['email' => $email, 'is_active' => 1])->row_array();
